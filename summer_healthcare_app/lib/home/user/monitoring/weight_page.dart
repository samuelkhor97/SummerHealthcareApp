...
class _WeightPageState extends State<WeightPage> {
  ...
  void initializePage() async {
    ...
    for (int i = 0; i < allWeights.length; i++) {
      TextEditingController weightController = TextEditingController(text: allWeights[i].weight);
  ...
  void showAddWeightPopUp() {
    ...
    String addWeightDate = DateTime.now().year.toString() + '/' + DateTime.now().month.toString() + '/' + DateTime.now().day.toString();
    showDialog<void>(
      context: context,
          ...
          actions: <Widget>[
            FlatButton(
              ...
              onPressed: () async {
                ...
                setState(() {});
                String token = await AuthService.getToken();
                await WeightServices().addWeight(headerToken: token, weight: weight.text, date: addWeightDate);
              },
            ),
          ],
        );
                            ...
                            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                              Column(
                                ...
                                children: [
                                  Text(
                                    allWeights[0].weight,
                                    style: TextStyle(fontSize: FontSizes.biggerText),
                                  ),
                                  SizedBox(
                                    height: Dimensions.d_5,
                                  ),
                                  Text('Start Weight', style: TextStyle(fontSize: FontSizes.normalText))
                                ],
                              ),
                              Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  ...
                                  Text('Current Weight', style: TextStyle(fontSize: FontSizes.normalText))
                                ],
                              )
                            ]),
